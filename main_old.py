    ...
    def replace(self, to_text, run_time):
        ...
        while True:
            ...
            for i in range(min(len(self.chars), len(to_chars))):
                pygame.draw.rect(window, "WHITE", (
                    transform(self.pixel_start[0]+self.chars[i][0]*self.pixel_size, to_pixel_start[0]+to_chars[i][0]*to_pixel_size, last),
                    transform(self.pixel_start[1]+self.chars[i][1]*self.pixel_size, to_pixel_start[1]+to_chars[i][1]*to_pixel_size, last),
...
text = Text("")
while True:
    text.replace(chr(random.randint(0x4e00, 0x9fff)), 3)
    time.sleep(1)
